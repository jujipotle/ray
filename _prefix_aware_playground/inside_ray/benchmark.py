...
async def async_request_openai_completions(
    ...
    async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
        ...
        headers = {
            ...
            "X-Prefix-ID": str(request_func_input.prefix_id),  # Add prefix_id to headers
        }
...
def sample_sharegpt_requests(
    ...
    for prompt, prompt_len, output_len in filtered_dataset:
        ...
        dataset_with_prefix_ids.append((prompt, prompt_len, output_len, prefix_map[prefix_key]))
    
    print(f"\nGenerated ShareGPT dataset statistics:")
    print(f"Total prompts: {len(dataset_with_prefix_ids)}")
    print(f"Total input tokens: {sum(prompt_len for _, prompt_len, _, _ in dataset_with_prefix_ids)}")
    print(f"Total output tokens: {sum(output_len for _, _, output_len, _ in dataset_with_prefix_ids)}")
    print(
    ...
    print(f"Average prompts per prefix: {len(dataset_with_prefix_ids) / len(prefix_map):.1f}\n")
...
async def benchmark(
    ...
    test_prompt, test_prompt_len, test_output_len, test_prefix_id = input_requests[0]  # Updated unpacking
