...
class PowerOfTwoChoicesReplicaScheduler(ReplicaScheduler):
    ...
    async def select_from_candidate_replicas(
        ...
        if chosen_replica_id is None:
            for r, queue_len in await self._probe_queue_lens(
                ...
                print(
                    f"[pow_2_scheduler.py: select_from_candidate_replicas] Probe Queue Length: Replica {r.replica_id} Queue Length: {queue_len}"
                )
            ...
            print(
                f"[pow_2_scheduler.py: select_from_candidate_replicas] Background Probing: Triggered for: {[r.replica_id for r in not_in_cache]}"
            )
        ...
        print(
            f"[pow_2_scheduler.py: select_from_candidate_replicas] Final Decision: Chosen Replica ID: {chosen_replica_id} with Queue Length: {lowest_queue_len}"
        )
    ...
    async def choose_replica_for_request(
        ...
        """
        print(
            f"[pow_2_scheduler.py: choose_replica_for_request] Choose replica for request"
        )
        try:
            if not is_retry:
