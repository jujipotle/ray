...
class RandomReplicaScheduler(ReplicaScheduler):
    ...
    async def _track_load_distribution(self):
        ...
        try:
            ...
            while True:
                ...
                #         # Construct full metric key including labels (to preserve le, model_name, etc.)
                #         label_suffix = ",".join(f"{k}={v}" for k, v in sorted(labels.items()))
                #         metric_key = f"{name}{{{label_suffix}}}" if label_suffix else name
                ...
                if self._num_requests_seen > 10 and total_load == 0:
                    ...
                    if self._zero_load_count >= 10:
                        ...
                        with open(os.path.join(results_dir, f"random_{int(time.time())}.json"), "w") as f:
                        ...
                        with open(os.path.join(prefix_match_dir, f"random_{int(time.time())}.json"), "w") as f:
                        ...
                        with open(os.path.join(metrics_dir, f"random_{int(time.time())}.json"), "w") as f:
